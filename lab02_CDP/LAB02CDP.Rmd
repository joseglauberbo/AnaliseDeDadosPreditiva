...
```
  Daí podemos analisar que: os dois modelos possuem os resíduos mais aglomerados perto do zero, porém os dois modelos possuem dispersões também, sendo o de 2010 mais homogêneo e com menos disperções.
  ...
  Em uma campanha, um número maior de fornecedores sugere que mais dinheiro foi aplicado aquela campanha, então podemos classificar que essa varíavel é bastante explicativa para o número de votos. A variável total_despesa é complementativa ao número de fornecedores, então elas duas são bastantes explicativas para o nosso modelo. 

```{r}
model_eleicoes_2006 <- lm(formula = votos ~ ., eleicoes_data_2006 %>% select(-ano, -cargo, -sequencial_candidato, -nome, -votos), na.action = na.omit)
