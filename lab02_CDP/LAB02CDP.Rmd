...
1) Um modelo de regressão múltipla com todas as variáveis é plausível para explicar a variação em y (número de votos) em 2006? Mesma pergunta para 2010.
  ...
  Podemos perceber, através da descrição dos dados, que algumas variáveis não influenciam na variável de saída (número de votos), essas são: nome, sequencial_candidato, e cargo. Essa última não influencia no modelo, pois ela é a mesma para todas as nossas observações, ou seja, deputado federal. Iremos desconsiderar também setor_economico_despesa e setor_economica_receita pois essas irão atrapalhar nossa leitura e interpretação do estudo. Iremos criar um modelo de regressão linear sem essas vaiáveis para demonstrar.

```{r}
dadosEleicoesFiltrados <- eleicoes_data_2006 %>% 
    select(-nome, -sequencial_candidato, -cargo, -ano)
modelo <- lm(votos ~ ., dadosEleicoesFiltrados)
modelo %>% summary()
```
